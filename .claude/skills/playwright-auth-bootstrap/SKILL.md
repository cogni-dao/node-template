...
description: Capture a reusable signed-in browser session for a Cogni node's candidate (test) env so an agent can drive authed Playwright flows — screenshot the flighted UI, exercise authed pages — without re-prompting MetaMask on every run. Use when validating a node customization on `<slug>-test.cognidao.org`, capturing a new env's storageState, screenshotting a flighted candidate, or troubleshooting why a captured session no longer authenticates.
---

# Playwright Auth Bootstrap — Authed Playwright via CDP Attach

> Goal: capture a reusable signed-in browser session for your node's candidate URL (`https://<slug>-test.cognidao.org`) so an AI agent can drive authed Playwright flows — and screenshot your flighted customization — without re-prompting MetaMask on every run.
>
...
For launch agents: this is how you take a screenshot of your node customization on the flighted candidate URL after a successful flight + `/version` match.
...
.local-auth/
  ...
  <slug>-test.storageState.json        # <slug>-test.cognidao.org
  ...
```

Ensure your root `.gitignore` excludes the entire `/.local-auth/` directory. Never commit — these files hold live session cookies, and the profile dir holds the MetaMask vault.
...
1. In the debuggable Chrome, navigate to your node's candidate env (`https://<slug>-test.cognidao.org`).
...
4. **Leave that tab open**, then capture the session into `.local-auth/<slug>-test.storageState.json`. If your repo ships a capture script (`scripts/dev/capture-authed-state.mjs`), run it; otherwise capture via CDP attach from a small Playwright/`chrome-remote-interface` script that connects to `http://localhost:9222` and writes the page's cookies + origins to the storageState JSON.

## Using captured state

Two valid consumers:

**1. Ad-hoc validation / screenshots — `playwright-cli`** (preferred for one-off runs):

```bash
playwright-cli -s=validate state-load .local-auth/<slug>-test.storageState.json
playwright-cli -s=validate open https://<slug>-test.cognidao.org
playwright-cli -s=validate snapshot          # or: screenshot for a PNG of the flighted UI
...
const storageState = path.join(
  process.cwd(),
  ".local-auth/<slug>-test.storageState.json"
);

const browser = await chromium.launch();
const ctx = await browser.newContext({ storageState });
const page = await ctx.newPage();
await page.goto("https://<slug>-test.cognidao.org");
// already signed in — no MetaMask needed
```

The two paths read the **same JSON schema** — `playwright-cli state-save` and `@playwright/test` `storageState` are interchangeable. Pick by context: validation/screenshot runs use the CLI (zero artifacts, snapshot-driven); committed tests use the library API.
...
- **Captured state has no cookies for the expected domain:** you signed in before the capture ran. Make sure the tab is still open at the target URL when you capture.
- **Playwright runs but site redirects to signin:** cookie expired — recapture. _Or:_ the site stores auth in `localStorage` rather than cookies, and CDP-attach export reports `origins: 0`. In that case the captured `storageState.json` is insufficient; a dedicated Playwright authfile step (manual signin inside a Playwright-launched browser) may be needed.
...
2. Open Chrome normally from Applications / Spotlight / Dock — it launches with your real default profile.

## Scope / non-goals

- Per-developer primitive. Multi-tenant / credential-broker flows for production agents are out of scope.
